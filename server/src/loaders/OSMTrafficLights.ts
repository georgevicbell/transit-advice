...
export async function getTrafficStops(rc: RouteConfig): Promise<TrafficLightItem[]> {
    var trafficStops: TrafficLightItem[] = [];

    try {
        const bounds = rc.bounds.min.lat + "," + rc.bounds.min.lon + "," + rc.bounds.max.lat + "," + rc.bounds.max.lon
        const json = await overpass(`
        [out:json][timeout:25];
        node["highway"="stop"](bbox:`+ bounds + `);
        out geom;
        `)
        ...
        console.log({ 'stops loaded': lightsLoaded.length })
        trafficStops = lightsLoaded.map((item: any) => {
            if (rc.bufferedRoute) {
        ...
        trafficStops = trafficStops.filter((item: any) => { return item != undefined })
        console.log({ 'stops found': trafficStops.length })
